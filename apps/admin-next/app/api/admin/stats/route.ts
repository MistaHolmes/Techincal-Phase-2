...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const subscriptionRevenue = premiumAccesses.reduce(
      (acc, access) => acc + (access.subscription?.priceInCents || 0),
      0
    );
    const totalRevenueCents =
      subscriptionRevenue + (tipsAgg._sum.amountInCents || 0);
    ...
    const [newUsersThisWeek, newUsersLastWeek, newBlogsThisWeek, newBlogsLastWeek] =
      await Promise.all([
